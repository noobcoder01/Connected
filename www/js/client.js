...
let recStartTime;
let recElapsedTime;
let ConnectedTheme = 'neon'; // neon - dark - forest - ghost ...
...
function getHtmlElementsById() {
    ...
    themeSelect = getId('ConnectedTheme');
...
function welcomeUser() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Please join our Connected Video Chat Meeting',
        ...
        event.channel.onmessage = (msg) => {
            switch (event.channel.label) {
                case 'Connected_chat_channel':
                    ...
                    break;
                case 'Connected_file_sharing_channel':
...
/**
 * Set Connected theme neon - dark - forest - sky - ghost
 * @param {*} theme
 */
function setTheme(theme) {
    if (!theme) return;

    ConnectedTheme = theme;
    switch (ConnectedTheme) {
        case 'neon':
            // neon theme
...
function setChatRoomBtn() {
    ...
    msgerTheme.addEventListener('click', (e) => {
        if (ConnectedTheme == 'ghost') return;
...
function createChatDataChannel(peer_id) {
    chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('Connected_chat_channel');
...
function createFileSharingDataChannel(peer_id) {
    fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('Connected_file_sharing_channel');
...
function selectFileToShare() {
    ...
    Swal.fire({
        allowOutsideClick: false,
        background: swalBackground,
        imageAlt: 'Connected-file-sharing',
    ...
    if (isImageURL(incomingFileInfo.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                imageAlt: 'Connected-file-img-download',
                showDenyButton: true,
        ...
        Swal.fire({
            allowOutsideClick: false,
            background: swalBackground,
            imageAlt: 'Connected-file-download',
            imageUrl: fileSharingImg,
            position: 'center',
            title: 'Received file',
            text: incomingFileInfo.fileName + ' size ' + bytesToSize(incomingFileInfo.fileSize),
